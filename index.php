...
<body>
    ...
    <div id="main">
                        ...
                        if( $j>= $startDay){
                            if( !empty( $sd[ $month][ $days])){
                                ...
                                if( ( $sd[ $month][ $days]) == "啟祥生日"){
                                    echo "<td id='BD'><div>" . $days . "</div><br><div>" . $str . "</div></td>";
                                }else{
                                    echo "<td id='sp'><div>" . $days . "</div><br><div>" . $str . "</div></td>";
                                }
                            }else if( ( "$year-$month-$days") == date( "Y-n-j")){
                                echo "<td id='TD'><div>" . $days . "</div><br><div>" . $str . "</div></td>";
                            }else{
                                echo "<td><div>" . $days . "</div><br><div>　</div></td>";
                            }
                            $str="";
                            $days++;
                        }else{
        ...
        <div id="year">
            <p id="link"><a  href="index.php?month=<?=$month;?>&year=<?=( $year- 1)?>">上一年</a></p>
            <p id="link"><a  href="index.php?month=<?=$month;?>&year=<?=( $year+ 1)?>">下一年</a> </p>
            <p id="link"><a  href="index.php?month=<?=$month;?>&year=<?=( date( "Y"));?>">今年</a></p>
            <?php
                if(( $month- 1)> 0){
                ?>
                    <p id="link"><a  href="index.php?month=<?=( $month- 1);?>&year=<?=$year?>">上一月</a><br></p>
                <?php
                }else{
                ?>
                    <p id="link"><a  href="index.php?month=<?=12;?>&year=<?=( $year- 1)?>">上一月</a><br></p>
                <?php
                }
                if(( $month+ 1)<= 12){
                ?>
                    <p id="link"><a  href="index.php?month=<?=( $month+ 1);?>&year=<?=$year?>">下一月</a></p>
                <?php
                }else{
                ?>
                    <p id="link"><a  href="index.php?month=<?=1;?>&year=<?=( $year+ 1)?>">下一月</a></p>
                <?php
                }
                ?>
                <p id="link"><a  href="index.php?month=<?=(date('m'));?>&year=<?=(date('Y'));?>">今天</a> </p>
